...
- [x] First trading-period access performs one enrichment fetch; repeated and
  concurrent successful access is cached.
- [x] Failed enrichment preserves base metadata and can be retried.
- [x] Auth tests cover cookie timeout, crumb 429, transient crumb failure,
  target 429, SOCKS proxy preservation, and no proxy overwrite.
- [x] Auth tests cover non-transient propagation, alternate retry, no empty
...
- [x] Existing history, auth, repair, WebSocket, cache, and integration tests
  remain green.
...
- [x] `make docs-build`
- [x] `git diff --check`
- [x] Verify local commands match `.github/workflows/test.yml` and
  `.github/workflows/docs.yml`.
- [x] Push the branch, verify the release PR is mergeable, and require remote
  test/lint success.
